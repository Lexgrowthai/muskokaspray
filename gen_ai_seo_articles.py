# -*- coding: utf-8 -*-
"""Generate 10 AI SEO insight articles for /insights/ai-seo/."""
import os, re

DIR = "/home/user/muskokaspray/insights/ai-seo"
BASE = "https://lexscale.ai/insights/ai-seo"

STYLE = r"""*{margin:0;padding:0;box-sizing:border-box;}
...
.comp-table .tag-chip{display:inline-block;background:rgba(106,92,255,.1);color:var(--pu);font-size:11px;font-weight:700;padding:3px 9px;border-radius:100px;}
...
.pa-cols{display:grid;grid-template-columns:1fr 1fr;gap:20px;margin:24px 0;}
.pa-col{background:rgba(255,255,255,.9);border:1px solid rgba(106,92,255,.1);border-radius:16px;padding:22px;}
.pa-col-h{font-size:14px;font-weight:800;color:var(--navy);margin-bottom:12px;display:flex;align-items:center;gap:8px;}
.pa-col-h::before{content:'';width:10px;height:10px;border-radius:3px;background:var(--pu);flex-shrink:0;}
.pa-col-list{display:flex;flex-direction:column;gap:7px;}
.pa-col-li{font-size:13px;color:#64748b;display:flex;align-items:center;gap:7px;}
.pa-col-li::before{content:'\2192';color:var(--pu);font-weight:700;font-size:12px;}
.shift-timeline{display:flex;flex-direction:column;gap:0;margin:28px 0;}
.st-row{display:grid;grid-template-columns:100px 1fr;gap:20px;align-items:start;padding:20px 0;border-bottom:1px solid rgba(106,92,255,.07);}
.st-row:last-child{border-bottom:none;}
.st-year{font-size:13px;font-weight:800;color:var(--pu);letter-spacing:-.3px;}
.st-desc{font-size:14px;color:#374151;line-height:1.65;}
.st-label{font-size:12px;font-weight:700;color:var(--navy);margin-bottom:4px;}
...
NAV = """<nav>
  ...
  <ul class="nav-links">
    ...
    <li class="has-drop">
      ...
      <div class="dropdown">
        <a href="/insights/chatgpt" class="drop-item"><div class="drop-ico"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#6A5CFF" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg></div><div><div class="drop-label">ChatGPT for Law Firms</div><div class="drop-sub">10 articles</div></div></a>
        ...
        <a href="/insights/perplexity" class="drop-item"><div class="drop-ico"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#20B8CD" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M12 8v4l3 3"/></svg></div><div><div class="drop-label">Perplexity for Law Firms</div><div class="drop-sub">2 articles</div></div></a>
        ...
        <a href="/insights/ai-receptionists" class="drop-item"><div class="drop-ico"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#6A5CFF" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 013.07 8.81 19.79 19.79 0 01.1 2.18 2 2 0 012.08 0h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L6.91 7.91a16 16 0 006.16 6.16l1.27-1.27a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16.92z"/></svg></div><div><div class="drop-label">AI Receptionists for Law Firms</div><div class="drop-sub">5 articles</div></div></a>
        ...
        <a href="/insights/entity-seo" class="drop-item"><div class="drop-ico"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#6A5CFF" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5"/></svg></div><div><div class="drop-label">Entity SEO &amp; Structured Data</div><div class="drop-sub">5 articles</div></div></a>
        <a href="/insights/ai-websites" class="drop-item"><div class="drop-ico"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#6A5CFF" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9h18M9 21V9"/></svg></div><div><div class="drop-label">AI Websites for Law Firms</div><div class="drop-sub">5 articles</div></div></a>
      </div>
    </li>
  </ul>
  <button class="nav-cta" onclick="document.getElementById('form-overlay').style.display='flex'">Book A Demo</button>
...
<p style="text-align:center;padding:8px 0;font-size:12px;color:rgba(255,255,255,.35);"><a href="/contact" style="color:rgba(255,255,255,.45);text-decoration:none;margin:0 8px;">Contact</a> &middot; <a href="/privacy" style="color:rgba(255,255,255,.45);text-decoration:none;margin:0 8px;">Privacy Policy</a></p>
</footer>"""

STICKY_MODAL_JS = """<div id="sticky-cta">
  <div class="sc-left">
    <div style="width:36px;height:36px;border-radius:50%;background:rgba(212,175,55,.15);border:1px solid rgba(212,175,55,.3);display:flex;align-items:center;justify-content:center;flex-shrink:0;">
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#D4AF37" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
    </div>
    <span class="sc-text"><strong>Is Your Firm Visible in AI Search?</strong> Find out where you stand &mdash; free AI visibility audit.</span>
  </div>
  <div class="sc-right">
    <button onclick="openForm()" style="background:linear-gradient(135deg,var(--gold3),var(--gold2));color:var(--navy);border:none;padding:10px 22px;border-radius:100px;font-size:13px;font-weight:700;cursor:pointer;font-family:inherit;white-space:nowrap;">Get Free AI Audit &rarr;</button>
...
<div id="form-overlay" onclick="if(event.target===this)closeForm()" style="position:fixed;inset:0;background:rgba(6,9,15,.75);backdrop-filter:blur(8px);z-index:1000;display:none;align-items:center;justify-content:center;padding:20px;">
  <div style="background:#fff;border-radius:24px;padding:40px;max-width:500px;width:100%;position:relative;box-shadow:0 24px 80px rgba(0,0,0,.3);max-height:90vh;overflow-y:auto;">
    <button onclick="closeForm()" style="position:absolute;top:18px;right:18px;background:rgba(11,21,54,.06);border:none;width:34px;height:34px;border-radius:50%;font-size:18px;cursor:pointer;display:flex;align-items:center;justify-content:center;color:#64748b;">&times;</button>
    ...
    <p style="font-size:14px;color:#64748b;margin-bottom:24px;line-height:1.6;">We'll run a full AI visibility audit across ChatGPT, Perplexity, and Google AI &mdash; and show you exactly where your firm stands.</p>
    <form onsubmit="submitForm(event)" style="display:flex;flex-direction:column;gap:14px;">
      ...
      <div><label style="display:block;font-size:11px;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:.6px;margin-bottom:6px;">Your Website URL</label><input type="url" placeholder="https://yourfirm.com" style="width:100%;border:1.5px solid rgba(106,92,255,.15);border-radius:10px;padding:12px 14px;font-size:14px;font-family:inherit;color:var(--navy);outline:none;box-sizing:border-box;"/></div>
      <button type="submit" style="background:linear-gradient(135deg,var(--pu),var(--pu2));color:#fff;border:none;padding:15px;border-radius:12px;font-size:15px;font-weight:700;cursor:pointer;font-family:inherit;margin-top:4px;box-shadow:0 4px 20px rgba(106,92,255,.3);">Run My Free AI Audit &rarr;</button>
    </form>
...
function toggleFaq(el){
  ...
  if(!isOpen){item.classList.add('open');ans.style.maxHeight='400px';}
}
...
RELATED_SIDEBAR = """    <div class="sidebar-card">
...
PLUS_ICON = '<div class="faq-icon"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#6A5CFF" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg></div>'


def p(*paras):
    return "".join('<p class="art-p">%s</p>\n' % t for t in paras)

def ul(items):
    return '<ul class="art-ul">\n' + "".join('<li class="art-li">%s</li>\n' % i for i in items) + '</ul>\n'

def callout(kind, label, text):
    return '<div class="callout %s"><div class="callout-label">%s</div><div class="callout-text">%s</div></div>\n' % (kind, label, text)

def query_grid(items):
    icon = '<div class="query-icon"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg></div>'
    cards = "".join('<div class="query-card">%s<div class="query-text">%s</div></div>\n' % (icon, t) for t in items)
    return '<div class="query-grid">\n%s</div>\n' % cards

def factors_grid(items):
    cards = ""
    for i, (h, d) in enumerate(items, 1):
        cards += '<div class="factor-card"><div class="factor-num">%02d</div><div class="factor-h">%s</div><div class="factor-p">%s</div></div>\n' % (i, h, d)
    return '<div class="factors-grid">\n%s</div>\n' % cards

def pa_cols(cols):
    out = ""
    for title, items in cols:
        lis = "".join('<div class="pa-col-li">%s</div>' % i for i in items)
        out += '<div class="pa-col"><div class="pa-col-h">%s</div><div class="pa-col-list">%s</div></div>\n' % (title, lis)
    return '<div class="pa-cols">\n%s</div>\n' % out

def shift_timeline(rows):
    colors = ["var(--pu)", "var(--gold3)", "var(--pu)", "var(--gold3)"]
    out = ""
    for i, (yr, label, desc) in enumerate(rows):
        c = colors[i % len(colors)]
        out += '<div class="st-row"><div class="st-year" style="color:%s;">%s</div><div><div class="st-label" style="color:%s;">%s</div><div class="st-desc">%s</div></div></div>\n' % (c, yr, c, label, desc)
    return '<div class="shift-timeline">\n%s</div>\n' % out

def comp_table(head, rows):
    th = "".join('<th>%s</th>' % h for h in head)
    body = ""
    for row in rows:
        tds = "".join('<td>%s</td>' % c for c in row)
        body += '<tr>%s</tr>\n' % tds
    return '<table class="comp-table"><thead><tr>%s</tr></thead><tbody>\n%s</tbody></table>\n' % (th, body)

def faq_block(pairs):
    items = ""
    for q, a in pairs:
        items += ('<div class="faq-item"><div class="faq-q" onclick="toggleFaq(this)">'
                  '<span class="faq-q-text">%s</span>%s</div>'
                  '<div class="faq-a"><div class="faq-a-inner"><p class="faq-a-text">%s</p></div></div></div>\n'
                  % (q, PLUS_ICON, a))
    return '<div class="faq-list">\n%s</div>\n' % items

def section(sid, heading, body):
    return ('<div class="art-section" id="%s">\n<h2 class="art-h2 with-bar">%s</h2>\n%s</div>\n'
            % (sid, heading, body))


def build(slug, title, desc, h1_main, h1_gold, deck, toc, sections, faqs, stats):
    url = "%s/%s" % (BASE, slug.replace(".html", ""))
    full_title = "%s | LexScale.ai" % title
    toc_html = ""
    for i, t in enumerate(toc, 1):
        toc_html += '<a href="#s%d" class="toc-item"><span class="toc-num">%02d</span><span class="toc-text">%s</span></a>\n' % (i, i, t)
    toc_html += '<a href="#s9" class="toc-item"><span class="toc-num">09</span><span class="toc-text">FAQs</span></a>\n'

    sections_html = ""
    for i, (heading, body) in enumerate(sections, 1):
        sections_html += section("s%d" % i, heading, body)

    faq_section = ('<div class="art-section" id="s9">\n<h2 class="art-h2">Frequently Asked Questions</h2>\n%s</div>\n'
                   % faq_block(faqs))

    stats_html = ""
    for i, (val, lbl) in enumerate(stats):
        if i > 0:
            stats_html += '<div class="sh-divider"></div>'
        fs = ' style="font-size:28px;"' if len(val) > 5 else ''
        stats_html += '<div class="stat-highlight"><div class="sh-val"%s>%s</div><div class="sh-lbl">%s</div></div>' % (fs, val, lbl)
    ...
    breadcrumb_ld = ('{\n  "@context": "https://schema.org",\n  "@type": "BreadcrumbList",\n  "itemListElement": [\n'
                     ...
                     '    {"@type": "ListItem","position": 3,"name": "AI SEO for Law Firms","item": "https://lexscale.ai/insights/ai-seo"},\n'
...
{faq_section}
    <div class="cta-banner">
      ...
      <p class="cb-p">LexScale.ai helps law firms build the authority, content, and AI visibility needed to compete in today's search landscape &mdash; and in the one that's coming next.</p>
      <div class="cb-btns">
...
{stats_html}
    ...
    <div class="sidebar-card gold-card">
      ...
      <p style="font-size:13px;color:#64748b;line-height:1.65;margin-bottom:16px;">Get a free AI visibility audit and see exactly where your firm stands across ChatGPT, Gemini, and Perplexity.</p>
...
{sticky}
        ...
        full_title=full_title, desc=desc, url=url, article_ld=article_ld,
        breadcrumb_ld=breadcrumb_ld, style=STYLE, nav=NAV, h1_main=h1_main,
        h1_gold=h1_gold, deck=deck, sections_html=sections_html,
        faq_section=faq_section, toc_html=toc_html, stats_html=stats_html,
        related=RELATED_SIDEBAR, footer=FOOTER, sticky=STICKY_MODAL_JS)
    ...
    assert html.count('<meta name="description"') == 1, slug
    links = len(re.findall(r'href="/[^"]+"', html))
    assert links >= 5, slug
    print("wrote %s (%d lines, %d internal links)" % (slug, html.count("\n")+1, links))
    return url
